...
PAGES = (
    Page(
        ...
        None,
        "Artifact result",
        "Generated by the default artifact workflow from the supplied inputs.",
    ),
    Page(
        3,
    ...
    Page(
        ...
        None,
        "Artifact result",
        "Selected Llama 128-GPU and vLLM comparisons. Grok 4k is omitted "
...
def make_fig6_result() -> Image.Image:
    fig, axes = plt.subplots(2, 2, figsize=(12.0, 5.3), constrained_layout=True)
    ...
    rx, ry = read_xy(
        ROOT / "results/reproduced/fig6/final_4nic_latency.csv",
        "L",
        "runtime",
    )
    plot_pair(
    ...
    rx, ry = read_bandwidth(
        ROOT / "results/reproduced/fig6/final_4nic_bandwidth.csv"
    )
    plot_pair(
        axes[1, 0],
        px,
        ...
        rx,
        ry,
        "Llama, 128 GPUs: bandwidth",
        "Bandwidth [Gbps]",
    )
    axes[1, 0].set_xscale("log")

    px, py = read_xy(
        ROOT / "data/output/vllm_llama8b_128tok/latency_runtime.csv",
        "L",
        "runtime",
    )
    rx, ry = read_xy(
        ROOT / "local_artifact/results/fig6/vllm/full_goal_latency_runtime.csv",
        "L",
        "runtime",
    )
    plot_pair(
    ...
    px, py = read_bandwidth(
        ROOT / "data/output/vllm_llama8b_128tok/bandwidth_runtime.csv"
    )
    rx, ry = read_bandwidth(
        ROOT / "local_artifact/results/fig6/vllm/full_goal_bandwidth_runtime.csv"
    )
    plot_pair(
        axes[1, 1],
        px,
...
def result_image(page: Page, work: Path, dpi: int) -> Image.Image:
    ...
    if page.result_kind == "fig6":
        return make_fig6_result()
