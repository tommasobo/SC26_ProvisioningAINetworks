...
ax_top.plot(full_L/1e3,full_T/1e6,color=C_COMP,ls="-",lw=1.6,label="Composite LP", zorder=3)
...
dx=np.diff(full_L/1e3); dy=np.diff(full_T/1e6)
s=np.where(dx>0,dy/dx,0); xm=(full_L[:-1]+full_L[1:])/2e3
