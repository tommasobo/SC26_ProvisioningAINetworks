...
class SerializedGraph {
  ...
  typedef std::vector<graph_node_properties> nodelist_t;
	...
	void DumpStuckNodes() {
		uint32_t num_nodes_val = (uint32_t) *((uint32_t*) mapping_start);
		int SIZEOF_NODE_INFO = sizeof(char) + sizeof(uint64_t) + sizeof(uint32_t)*7 + sizeof(uint8_t)*2;
		int stuck_count = 0;
		for (uint32_t i = 0; i < num_nodes_val; i++) {
			char* start_of_node = mapping_start + sizeof(uint32_t)*2 + sizeof(uint32_t)*num_root_nodes + SIZEOF_NODE_INFO*i;
			uint32_t dep_cnt = (uint32_t) *((uint32_t*) start_of_node);
			if (dep_cnt > 0) {
				stuck_count++;
				if (stuck_count <= 20) {
					char type = *(start_of_node + sizeof(uint32_t));
					uint8_t proc = *((uint8_t*)(start_of_node + sizeof(uint32_t) + sizeof(char) + sizeof(uint32_t) + sizeof(uint64_t) + sizeof(uint32_t)));
					uint32_t peer = *((uint32_t*)(start_of_node + sizeof(uint32_t) + sizeof(char)));
					uint64_t size = *((uint64_t*)(start_of_node + sizeof(uint32_t) + sizeof(char) + sizeof(uint32_t)));
					uint32_t tag = *((uint32_t*)(start_of_node + sizeof(uint32_t) + sizeof(char) + sizeof(uint32_t) + sizeof(uint64_t)));
					const char* type_str = (type == 1) ? "SEND" : (type == 2) ? "RECV" : "CALC";
					fprintf(stderr, "[STUCK rank %d] node %u: type=%s dep_cnt=%u cpu=%u peer=%u size=%lu tag=%u\n",
							my_rank, i, type_str, dep_cnt, proc, peer, (unsigned long)size, tag);
				}
			}
		}
		if (stuck_count > 0)
			fprintf(stderr, "[STUCK rank %d] Total: %d stuck nodes out of %u\n", my_rank, stuck_count, num_nodes_val);
	}
